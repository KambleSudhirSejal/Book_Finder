...
const Hero = ({ searchQuery }) => {
    ...
    const fetchBooks = async () => {
      try {
        ...
        if (limitedBooks.length < 35) {
          const missingCoount = 35 - limitedBooks.length;
          const dummyBooks = Array.from({ length: missingCoount }, (_, i) => ({
            dummy: true,
            id: `dummy-${i}`,
          }));
  ...
  if (loading) {
    return (
      <div className="animate-pulse grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 p-6">
        {[...Array(6)].map((_, i) => (
          <div
            key={i}
            className="h-64 md:h-96 bg-gray-800/50 rounded-xl md:rounded-2xl
                backdrop-blur-lg "
          />
        ))}
      </div>
    ...
    return (
      <div className=" min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 p-8 flex justify-center items-center">
        <p className=" text-cyan-300 text-lg md:text-xl text-center ">
          Nobooks found matching your search.
        </p>
      </div>
    );
  }
  ...
  return (
    <div
      className="min-h-screen pt-16 md:pt-20 bg-gradient-to-b from-gray-900 to-gray-800
    p-4 sm:p-6 md:p-8"
    >
      <div className="max-w-7xl mx-auto">
          ...
          {paginatedBooks.map((book, index) => {
            if (book.dummy) {
              return (
                <div
                  key={book.id || index}
                  className="group relative bg-gray-800/30 rounded-xl md:rounded-2xl backdrop-blur-xl border-dashed border-gray-700/50 flex items-center justify-center h-96 md:h-96"
                >
            ...
            return (
              <div
                id="hero"
                key={book.id}
                className="group relative bg-gray-800/30 rounded-xl md:rounded-2xl
                 backdrop-blur-xl border border-white/10 hover:border-cyan-400/30 transition-all duration-300
                 hover:-translate-y-1 md:hover:-translate-y-2 shadow-lg hover:shadow-xl md:shadow-2xl
              ...
              >
                <a
                  ...
                  className="adsolute inset-0 z-10 cursor-pointer"
                  aria-label={`View ${book.title}`}
                ></a>
                <div className="p-4 md:p-6">
                  <div
                    className="relative aspect-[4/5] w-full rounded-xl md:rounded-2xl
                    overflow-hidden bg-gradient-to-br from-cyan-400/10 to-blue-400/10"
                  >
                    <img
                    ...
                    />
                    <div
                      className="absolute inset-0 bg-gradient-to-t from-gray-900/80
                    ...
                    <div className="mt-2 md:mt-4 flex flex-wrap gap-1 md:gap-2">
                      {book.categories
                        ...
                        .map((category, index) => (
                          <span
                            key={index}
                            className="px-2 py-1 md:px-3 md:py-1 rounded-full bg-gray-700/50
                            text-xs text-cyan-300 backdrop-blur-sm "
                          >
                            {category.trim()}
                            {index === 2 &&
                ...
                <div
                  className="absolute inset-0 rounded-xl md:rounded-2xl bg-gradient-to-br from-cyan-400/20 to-blue-500/20
        ...
        <div className="mt-6 md:mt-8 flex-wrap justify-center items-center gap-2 md:gap-3">
          <button
            onClick={() => setCurrentPage((p) => Math.max(1, p - 1))}
            disabled={currentPage === 1}
            className="px-3 py-1 md:px-4 md:py-2 rounded-md md:rounded-lg bg-gray-900/80 border
            border-gray-700/50 text-cyan-300 text-xs md:text-sm hover:bg-gray-900/60 transition-colors
            disabled:opacity-50"
          >
            Previous
          </button>
          {generatePageNumber().map((page, index) => (
            <button
              ...
              className={`px-3 py-1 md:px-4 md:py-4 rounded-md md:rounded-lg text-xs md:text-sm 
                    ${
                      page === currentPage
